...
export default function ProviderViewPlacement() {
  const { id } = useParams();
  const navigate = useNavigate();
  const [p, setP] = useState(null);

  useEffect(() => {
    api.get(`/provider/placements/${id}`).then(({ data }) => setP(data)).catch(() => navigate('/provider/requests'));
  }, [id]);
  ...
  return (
    <div>
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.5rem', flexWrap: 'wrap', gap: '1rem' }}>
        <button className="btn btn-ghost btn-sm" onClick={() => navigate('/provider/requests')}>← Back to Requests</button>
      </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '1.5rem' }}>
        ...
        {p.providerFeedback && (
          <div className="panel" style={{ gridColumn: '1/-1' }}>
            <div className="panel-header"><h3>Your Feedback</h3></div>
            <div style={{ padding: '1.25rem', color: 'var(--text)', fontSize: '0.9375rem', lineHeight: 1.7, whiteSpace: 'pre-wrap' }}>{p.providerFeedback}</div>
          </div>
        )}
